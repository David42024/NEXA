...
export default function NexabotCopilot({
  ...
  const urgencyCopy = dias == null
    ? null
    : urgent
    ? `Prioridad alta · ${dias} días para agotar datos`
    : `${dias} días para agotar datos`
  ...
  return (
    <aside className="flex flex-col rounded-xl border border-black/60 bg-white text-navy-900 shadow-sm transition-colors xl:sticky xl:top-6 xl:h-fit dark:border-white/60 dark:bg-navy-900 dark:text-slate-200">
      ...
      <div className="flex-1 space-y-3 px-4 py-3">
        {/* Urgencia */}
        {urgencyCopy && (
          <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-400/30 dark:bg-amber-500/15">
            <Hourglass className="h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-300" />
            <p className="text-xs font-semibold text-amber-700 dark:text-amber-200">{urgencyCopy}</p>
          </div>
        )}
